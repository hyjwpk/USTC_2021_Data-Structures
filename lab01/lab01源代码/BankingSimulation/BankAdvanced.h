...
public:
	 ...
	 * @param listNum 队列总数
	*/
	BankAdvanced(const int totalMoney, const int closeTime, const int moneyScope, const int durationScope, const int intertimeScope, const int listNum);

	/**
	 * @brief 处理事件
	 * @details data[1]存储事件种类Type，0-到达事件， 1-withdrawMoneyList离开事件，n-customerList[n-2]离开事件
...
private:
	...
	*/
	 ...
	 * @param type 1-withdrawMoneyList n-customerList[n-2]
	*/
